...
div(2,1) #2
/("Hello ", "world") #Error 
x = 2; x ^= 4 # ; works to run several commands on the line
...
#1
boolvector = fill(true,10)

boolvector = Bool(10,1)

# undef basically means fill the array or matrix with values of this type, but dont define the value.
Vector{Bool}(undef,10)

#2
sr = 1.0:(3.2-1.0)/17:3.2
v = collect(sr)
length(v)



# You can specify either the length of the stop
# the ... is called the splat operator and it works like collect
# writing [] around it makes it a vector
[ range(1, stop=3.2, length=18)... ]


#3
splice_vector = [1,2,3,4,5]
splice!(splice_vector, 2, 4)
splice!(splice_vector, 4, 2)

splice_vector

# 4 Filter function
nr = [1,2,3,6,7,3,1,10,28]
even_numbers = filter(x -> iseven(x), nr)
even_numbers = filter(iseven, nr)

# 5 Difference between max and maximum
# Max finds the maximum value in a cell while maximum finds the cells / rows with the highest values combined.
max(2,3,4) # Finds the maximum of the Arguments
maximum([2,3,4]) # Finds the maximum in the object you give it
...
length(append!(var1,var2))


# 7
#Using `find()`, equality `==`, and vectorized functions
hector_the_vector = [10,11,1,4,5,1,11,1,3,0,2,-1]
findall()

indk = findall(x -> x == 1, hector_the_vector)

hector_the_vector.==1

# 8
# Write ?sort! in the REPL
...
A  # Fucking magic and also slightly dangerous maybe
...
# So this copies everything from A into B
B[1] = 100
B
copyto!(B, A)
B
...
# It returns the second value of the first is not found
get(d, "r1", NaN)
get(d, "hi", NaN)
...
#1 Create a 10 by 10 matrix and fill it with floats
A = zeros(10,10)

B = Matrix{Int64}(undef,10, 10)

C = [i == j ? 1 : 0  for i in 1:10, j in 1:10]


#2 fill the diagonal with 10
# This code writes 
#If i is equal to j then if true, write i and if not write zero, do that for all the following values of i and j.
A = [i == j ? i : 0  for i in 1:10, j in 1:10]


#3 create a range from 10 to 1
sr = 10:-1:1
v = collect(sr)

# Filling a matrix with it
test_4 = [j for i in 1:5, j in 10:-1:1]

# Solution with repeat and transpose
repeat(transpose(10:-1:1),5,1)

#4 Create 3 dimensional array (5,5,3)
A = [i*j*h for i in 1:5, j in 1:5, h = 1:3]
test_5 = [(i*j)^h for i in 1:5, j in 1:5, h = 1:3]

#5 Create a dictionary
Alphabet = "ABCDEFGHIJKLMNOPQRSTUVXYZ"
d = Dict(i => Alphabet[i] for i = 1:1:25)
d[1]
...
# Does it check what the mean number of times the number is odd?
mean(isodd, x)
...
"""
#1
# So much smarter to create it this way
Rnr = Random.randexp(1_000,10_000)

# Now I want to estimate the standard deviation from 1.0 for each column, and apparantly I should use mean?
mRnr = mean(Rnr .- 1 , dims = 1)

std(mRnr)

# 2
A = reshape([1:9...],3,3)
Ac = circshift(A,(1,1)) # THis function shifts every element in the matrix one row down and one row left.
Ar = rot180(A) # This function rotates the matrix 180 degrees so that position 1,1 goes to 3,3 and 1,2 goes to 3,2 and 1,3 goes to 3,1


B = rand(3,3,3)
Bc = circshift(B,(1,1,1))

# 3 get eigen values

matrix_test = rand(10,10)

eigenvalues = eigen(matrix_test).values
...
# if returns a value, no need for return
r = if x > 3
      "$x is greater than 3"
    elseif x == 3
      "$x is 3"
...
# THis is not a ternery operator, this is just 2 different variable assignments. 
x = 3; y = 2
...
# This is so cool, nested for loops in one line
...
# the eachindex(x) function is basically like saying range(1:length(x)) in python
v = ["a","b","c"]
for i = eachindex(v)
  println(i)
end
...
# enumerate() creates the pair of both the index and the value of V
for i = enumerate(v)
  println(i)
end

# assign the iteration and the value in the loop
# And you can create a for loop where you have both the value and the iteration at the same time O.O
for (i,v) = enumerate(v)
  println(i, v)
end
...
# you  can iterate over the keys in a dictionary with the keys() command
for k in keys(d)
  println(k+1)
end

# iterate over dictionary values
# You can also iterate over the values in a dictionary with the values command
for val in values(d)
  println(val)
end

# iterating over dictionary return tuple of (key, value)
# You can iterate over both the key and value with a normal for loop of a dictionary
for (k,v) in d
