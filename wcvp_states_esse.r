...
length(unique(renamed_occurence_subset$species)) 
...
# Add a new column to the csv file which is Tropical rainforest or not.
  ...
  # Now we add the WCVP climate description to the result_summary
 ...
 # How many of the species in result_summary have the climate description wet_tropical.
  ...
  Wet_tropical_widespread_percentage # 0.13
  ...
  Wet_tropical_non_trf_percentage # 0.17
  ...
  # We can now use these percentages to estimate how many of the Wet_tropical and non-wet tropical species are Trf, non-trf or widespread
  ...
  missing_sp <- setdiff(wcvp_accepted_species_subset$taxon_name, result_summary$wcvp_taxon_name)
 ...
 # Finding the climate description for the missing species
  ...
  # We need to find out how many of the species in the tree are Trf, non-trf and widespread
...
if (all(sp_not_in_tree %in% renamed_occurence_subset$wcvp_taxon_name)) { # If all the tip labels are found in the occurences use only them to create the presence absence matrix
  ...
  # Subset the occurrences to only include the species were missing from the tree
  renamed_occurence_subset <- renamed_occurence[which(renamed_occurence$wcvp_taxon_name %in% sp_not_in_tree),]
...
##################################################################################################################################################################
  ...
  renamed_occurence_subset <- renamed_occurence[which(renamed_occurence$wcvp_taxon_name %in% tree$tip.label),] # Subsetting the renamed_occurence dataset to only include the species in the tree
  ...
  occurrence_missing_sp <- setdiff(tree$tip.label, renamed_occurence_subset$wcvp_taxon_name) # Finding the species which are missing from the occurrences.
